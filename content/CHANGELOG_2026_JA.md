...
| 日本語 | English | Category |
|--------|---------|----------|
| `claude server` に永続セッションサポートを追加: `session_key` を持つ接続は WebSocket 切断後も維持され、サーバー再起動をまたいで再開可能。新フラグ: `--workspace`、`--idle-timeout`、`--max-sessions` | Added persistent session support to `claude server`: connections with a `session_key` survive WebSocket disconnects and can be resumed across server restarts. New flags: `--workspace`, `--idle-timeout`, `--max-sessions`. | added |
| ワークツリーまたは同一ディレクトリ分離による複数の同時セッションのホスティングに対応した `claude remote-control server` を追加 | Added `claude remote-control server` for hosting multiple concurrent sessions with worktree or same-dir isolation | added |
| `/remote-control` および `claude remote-control` に、claude.ai/code に表示されるカスタムセッションタイトルを設定するためのオプションの name 引数を追加（`/remote-control My Project` または `--name "My Project"`） | Added optional name argument to `/remote-control` and `claude remote-control` (`/remote-control My Project` or `--name "My Project"`) to set a custom session title visible in claude.ai/code | added |
| 音声STTが新たに10言語追加でサポート対象（計20言語）— ロシア語、ポーランド語、トルコ語、オランダ語、ウクライナ語、ギリシャ語、チェコ語、デンマーク語、スウェーデン語、ノルウェー語 | Added Voice STT support for 10 new languages (20 total) — Russian, Polish, Turkish, Dutch, Ukrainian, Greek, Czech, Danish, Swedish, Norwegian | added |
| ロゴとスピナーにエフォートレベルの表示（例: "with low effort"）を追加し、有効なエフォート設定を確認しやすく改善 | Added effort level display (e.g., "with low effort") to the logo and spinner, making it easier to see which effort setting is active | added |
| `claude --agent` 使用時のターミナルタイトルへのエージェント名表示を追加 | Added agent name display in terminal title when using `claude --agent` | added |
| `httpProxyPort` でカスタム MITM プロキシを使用する際に `gh`、`gcloud`、`terraform` などの Go プログラムが TLS 証明書を検証できるようにする `sandbox.enableWeakerNetworkIsolation` 設定を追加（macOS のみ） | Added `sandbox.enableWeakerNetworkIsolation` setting (macOS only) to allow Go programs like `gh`, `gcloud`, and `terraform` to verify TLS certificates when using a custom MITM proxy with `httpProxyPort` | added |
| Claude のシステムプロンプトから組み込みのコミット・PR ワークフロー指示を除去できる `includeGitInstructions` 設定（および `CLAUDE_CODE_DISABLE_GIT_INSTRUCTIONS` 環境変数）を追加 | Added `includeGitInstructions` setting (and `CLAUDE_CODE_DISABLE_GIT_INSTRUCTIONS` env var) to remove built-in commit and PR workflow instructions from Claude's system prompt | added |
| 再起動なしで保留中のプラグイン変更を有効化する `/reload-plugins` コマンドを追加 | Added `/reload-plugins` command to activate pending plugin changes without restarting | added |
| macOS および Windows で Claude Code Desktop を案内するスタートアップ時のプロンプトを追加（最大3回表示、非表示化可能） | Added a one-time startup prompt suggesting Claude Code Desktop on macOS and Windows (max 3 showings, dismissible) | added |
| スキルが SKILL.md コンテンツ内で自身のディレクトリを参照するための `${CLAUDE_SKILL_DIR}` 変数を追加 | Added `${CLAUDE_SKILL_DIR}` variable for skills to reference their own directory in SKILL.md content | added |
...
| フックイベントに `agent_id`（サブエージェント用）と `agent_type`（サブエージェントおよび `--agent` 用）を追加 | Added `agent_id` (for subagents) and `agent_type` (for subagents and `--agent`) to hook events | added |
| `--worktree` セッション実行時、ステータスラインフックコマンドに name・path・branch・元リポジトリディレクトリを含む `worktree` フィールドを追加 | Added `worktree` field to status line hook commands with name, path, branch, and original repo directory when running in a `--worktree` session | added |
| バックグラウンドタスクインジケーターへのスケジュール済みスキルトリガーの追加、およびスケジュールとカウントダウンを表示する詳細ダイアログを追加 | Added scheduled skill triggers to the background tasks indicator with a detail dialog showing schedule and countdown | added |
| インストール前に表示されるプラグイン信頼警告に組織固有のコンテキストを付加するため、マネージド設定に `pluginTrustMessage` を追加 | Added `pluginTrustMessage` in managed settings to append organization-specific context to the plugin trust warning shown before installation | added |
| Team プラン OAuth ユーザーにも Enterprise と同様にポリシー制限の取得（例: リモートコントロール制限）を追加 | Added policy limit fetching (e.g., remote control restrictions) for Team plan OAuth users, not just Enterprise | added |
| `strictKnownMarketplaces` に `hostPattern` 制限と併用してファイル/ディレクトリのマーケットプレイスソースを正規表現マッチングする `pathPattern` を追加 | Added `pathPattern` to `strictKnownMarketplaces` for regex-matching file/directory marketplace sources alongside `hostPattern` restrictions | added |
...
| 自動メモリディレクトリを直接パスで上書きするための環境変数 `CLAUDE_CODE_AUTO_MEMORY_PATH` を追加 | Added `CLAUDE_CODE_AUTO_MEMORY_PATH` env var to override the auto-memory directory with a direct path | added |
| MCP サーバー向けに、標準ディスカバリが失敗した場合のカスタム OAuth メタデータディスカバリ URL を指定する `oauth.authServerMetadataUrl` 設定オプションを追加 | Added `oauth.authServerMetadataUrl` config option for MCP servers to specify a custom OAuth metadata discovery URL when standard discovery fails | added |
| `acceptEdits` モードにおいて、シンボリックリンクされた親ディレクトリ経由で新規ファイルを書き込むことで作業ディレクトリを逸脱できるシンボリックリンクバイパスを修正 | Fixed symlink bypass where writing new files through a symlinked parent directory could escape the working directory in `acceptEdits` mode | fixed |
| 管理設定で `allowManagedDomainsOnly` が有効な場合、許可されていないドメインへのアクセス承認をユーザーに求めていた不具合を修正 — 非許可ドメインはバイパスなしで自動的にブロックされるように変更 | Fixed sandbox prompting users to approve non-allowed domains when `allowManagedDomainsOnly` is enabled in managed settings — non-allowed domains are now blocked automatically with no bypass | fixed |
| スキルの allowed-tools に列挙された場合に、インタラクティブツール（例: `AskUserQuestion`）が許可プロンプトをバイパスして空の回答で暗黙的に自動許可される問題を修正 | Fixed interactive tools (e.g., `AskUserQuestion`) being silently auto-allowed when listed in a skill's allowed-tools, bypassing the permission prompt and running with empty answers | fixed |
| ワーキングツリーに大きな未追跡バイナリファイルが存在する場合にコミット時に数GB規模のメモリスパイクが発生する問題を修正 | Fixed multi-GB memory spike when committing with large untracked binary files in the working tree | fixed |
| 入力ボックスに下書きテキストがある場合に Escape が実行中のターンを中断しない問題を修正。上矢印キーでキュー済みメッセージを編集用に引き戻し、Ctrl+U で入力行をクリア可能 | Fixed Escape not interrupting a running turn when the input box has draft text. Use Up arrow to pull queued messages back for editing, or Ctrl+U to clear the input line. | fixed |
| リモートコントロールセッションでローカルスラッシュコマンド（`/voice`、`/cost`）を実行したときにAndroidアプリがクラッシュする問題を修正 | Fixed Android app crash when running local slash commands (`/voice`, `/cost`) in Remote Control sessions | fixed |
| 長時間セッションで古いメッセージ配列バージョンが React Compiler の `memoCache` に蓄積するメモリリークを修正 | Fixed a memory leak where old message array versions accumulated in React Compiler `memoCache` over long sessions | fixed |
| 長いセッションでREPLのレンダースコープが蓄積されるメモリリークを修正（1000ターンで約35MB） | Fixed a memory leak where REPL render scopes accumulated over long sessions (~35MB over 1000 turns) | fixed |
| インプロセスのチームメートにおいて、チームメートの存続期間中に親の会話履歴全体がピン留めされ、`/clear` や自動コンパクト後のGCが妨げられていたメモリ保持の問題を修正 | Fixed memory retention in in-process teammates where the parent's full conversation history was pinned for the teammate's lifetime, preventing GC after `/clear` or auto-compact | fixed |
| インタラクティブモードで長時間のセッション中にフックイベントが無制限に蓄積されるメモリリークを修正 | Fixed a memory leak in interactive mode where hook events could accumulate unboundedly during long sessions | fixed |
| `--mcp-config` が破損ファイルを参照した際のハングを修正 | Fixed hang when `--mcp-config` points to a corrupted file | fixed |
| スキル・プラグインが多数インストールされている場合の起動が遅い問題を修正 | Fixed slow startup when many skills/plugins are installed | fixed |
| `cd <outside-dir> && rm/mv/cp ...` 実行時のパーミッションプロンプトが「`<dir>/` の読み取りを許可」のみを表示していた問題を修正し、連鎖する書き込みコマンドも表示されるよう対応 | Fixed `cd <outside-dir> && rm/mv/cp ...` permission prompt to mention the chained write command instead of only showing "allow reading from <dir>/" | fixed |
| `cd <outside-dir> && <cmd>` の権限確認プロンプトで「Yes, allow reading from <dir>/」のみ表示される問題を修正し、チェーンされたコマンドも正しく表示されるように対応 | Fixed `cd <outside-dir> && <cmd>` permission prompt to surface the chained command instead of only showing "Yes, allow reading from <dir>/" | fixed |
| 条件付き `.claude/rules/*.md` ファイル（`paths:` フロントマター付き）およびネストされた CLAUDE.md ファイルがプリントモード（`claude -p`）で読み込まれない問題を修正 | Fixed conditional `.claude/rules/*.md` files (with `paths:` frontmatter) and nested CLAUDE.md files not loading in print mode (`claude -p`) | fixed |
| `/clear` がすべてのセッションキャッシュを完全にクリアしない問題を修正、長時間セッションでのメモリ保持量を削減 | Fixed `/clear` not fully clearing all session caches, reducing memory retention in long sessions | fixed |
| スクロールバック境界でアニメーション要素が引き起こすターミナルのちらつきを修正 | Fixed terminal flicker caused by animated elements at the scrollback boundary | fixed |
| macOS で MCP サーバーと OAuth を使用した際の UI フレームドロップを修正（2.1.x からのリグレッション） | Fixed UI frame drops on macOS when using MCP servers with OAuth (regression from 2.1.x) | fixed |
| 同期デバッグログのフラッシュによりタイピング中に断続的に発生するフレームの停止を修正 | Fixed occasional frame stalls during typing caused by synchronous debug log flushes | fixed |
| `TeammateIdle` および `TaskCompleted` フックで `{"continue": false, "stopReason": "..."}` によるチームメート停止をサポートするよう修正（`Stop` フックの動作に準拠） | Fixed `TeammateIdle` and `TaskCompleted` hooks to support `{"continue": false, "stopReason": "..."}` to stop the teammate, matching `Stop` hook behavior | fixed |
| `WorktreeCreate` と `WorktreeRemove` プラグインフックが暗黙的に無視される問題を修正 | Fixed `WorktreeCreate` and `WorktreeRemove` plugin hooks being silently ignored | fixed |
| プラグインまたは SDK コンシューマーが登録した `WorktreeCreate` フックがサイレントに無視される問題を修正 | Fixed `WorktreeCreate` hooks registered by plugins or SDK consumers being silently ignored | fixed |
| スキルの説明にコロンを含む場合（例: "Triggers include: X, Y, Z"）にSKILL.mdフロントマターから読み込みに失敗する問題を修正 | Fixed skill descriptions with colons (e.g., "Triggers include: X, Y, Z") failing to load from SKILL.md frontmatter | fixed |
| `description:` フロントマターフィールドがないプロジェクトスキルが Claude の利用可能なスキル一覧に表示されない問題を修正 | Fixed project skills without a `description:` frontmatter field not appearing in Claude's available skills list | fixed |
| MCPサーバーのすべてのツールで同一のトークン数が表示される `/context` の問題を修正 (#なし) | Fixed `/context` showing identical token counts for all MCP tools from a server | fixed |
| Windows上でモデルがGit BashでCMDスタイルの`2>nul`リダイレクトを使用した際に`nul`ファイルが実際に作成されてしまう問題を修正 | Fixed literal `nul` file creation on Windows when the model uses CMD-style `2>nul` redirection in Git Bash | fixed |
| サブエージェントのトランスクリプト展開表示（Ctrl+O）で各ツール呼び出しの下に余分な空白行が表示される問題を修正 | Fixed extra blank lines appearing below each tool call in the expanded subagent transcript view (Ctrl+O) | fixed |
| `/config` の検索ボックスにフォーカスがあり空の場合、Tab/矢印キーで設定タブを循環できない問題を修正 | Fixed Tab/arrow keys not cycling Settings tabs when `/config` search box is focused but empty | fixed |
| サービスキー OAuth セッション（CCR コンテナ）がプロファイルスコープのエンドポイントへの 403 エラーにより `[ERROR]` ログを大量出力する問題を修正 | Fixed service key OAuth sessions (CCR containers) spamming `[ERROR]` logs with 403s from profile-scoped endpoints | fixed |
| 「Remote Control active」ステータスインジケーターの色が不統一だった問題を修正 | Fixed inconsistent color for "Remote Control active" status indicator | fixed |
| 入力途中でのディクテーション時に音声波形カーソルが最初のサフィックス文字を覆う問題を修正 | Fixed Voice waveform cursor covering the first suffix letter when dictating mid-input | fixed |
| ウォームアップ中に音声入力が約2個に制限せず5個全てのスペースを表示していた問題を修正（「keep holding…」ヒントとの整合性対応） | Fixed Voice input showing all 5 spaces during warmup instead of capping at ~2 (aligning with the "keep holding…" hint) | fixed |
| MCP バイナリコンテンツ処理の改善: PDF・Office ドキュメント・音声を返すツールが、生の base64 を会話コンテキストにダンプする代わりに、デコードされたバイトを正しいファイル拡張子でディスクに保存するよう変更。WebFetch もバイナリレスポンスをサマリーと共に保存。 | Improved MCP binary content handling: tools returning PDFs, Office documents, or audio now save decoded bytes to disk with the correct file extension instead of dumping raw base64 into the conversation context. WebFetch also saves binary responses alongside its summary. | improved |
| メッセージ更新間で `onSubmit` を安定させることで、長時間セッションのメモリ使用量を改善 | Improved memory usage in long sessions by stabilizing `onSubmit` across message updates | improved |
| LSP ツールのレンダリングとメモリコンテキスト構築を改善し、ファイル全体を読み込まないように変更 | Improved LSP tool rendering and memory context building to no longer read entire files | improved |
| セッションアップロードとメモリ同期を改善し、サイズ/バイナリチェック前に大容量ファイルをメモリへ読み込まないよう対応 | Improved session upload and memory sync to avoid reading large files into memory before size/binary checks | improved |
| 存在確認時のファイルコンテンツ読み込みを回避し、ファイル操作パフォーマンスを改善 (6 箇所) | Improved file operation performance by avoiding reading file contents for existence checks (6 sites) | improved |
| `--append-system-prompt-file` と `--system-prompt-file` がインタラクティブモードでも動作することを明確化するためドキュメントを改善（以前はプリントモードのみと記載されていた） | Improved documentation to clarify that `--append-system-prompt-file` and `--system-prompt-file` work in interactive mode (the docs previously said print mode only) | improved |
| Ctrl+O のシンキングサマリー表示を全内容ではなく `✻ Thinking…` スタブのみ表示するよう変更（元に戻すには settings.json で `showThinkingSummaries: true` を設定） | Changed thinking summaries in Ctrl+O to show a `✻ Thinking…` stub instead of full content. Set `showThinkingSummaries: true` in settings.json to restore. | changed |
| [VSCode] タイトルなしかつ空のアクティブセッションにリネームと削除アイコンが表示される問題を修正 | [VSCode] Fixed rename and remove icons showing on the current active session when it is untitled and empty | fixed |
| [VSCode] 一部のツールでタイムラインに空白のドットが表示される問題を修正 | [VSCode] Fixed blank dot appearing in the timeline for some tools | fixed |
