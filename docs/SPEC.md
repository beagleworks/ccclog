...
- 英語列ハイライトは一致トークン単位で適用（`<mark class="highlight-token">`、黄色背景）
- 日本語列ハイライトは一致箇所単位で適用（`<mark class="highlight-token">`、黄色背景）
  - 一致箇所が複数ある場合は全出現箇所をハイライト
  - 検索判定（空白正規化後の `includes`）とハイライト表示（生文字列一致）の差により、空白ゆれ一致時はハイライトが付与されない場合がある
- 検索結果がある月グループは自動展開
...
7. 当該年の記載済みバージョンのうち、GitHub CHANGELOG に存在しないバージョンを検出し、当該年ファイルから該当セクションを除去する（§5.2.4）
...
上流 CHANGELOG から一度公開されたバージョンが丸ごと削除される場合がある（npm には残存）。
この場合、ローカル CHANGELOG に残り続けるバージョンを自動除去する。

**検出条件**
- 当該年の `CHANGELOG_{YEAR}_JA.md` に存在するバージョンのうち、GitHub CHANGELOG（ステップ 2 で取得済み）に存在しないもの

**除去方法**
- `splitChangelogContent()` で分解した `sections` Map から該当バージョンを削除し、残りのセクションで再構築する

**レポート**
- 除去したバージョンを `removedVersions` としてレポートに含める
- コンソールに `[upstream_removed] vX.Y.Z は上流 CHANGELOG から削除されたため除去` を出力
...
|-----------|------------------------|
| `sync-versions.ts` | CHANGELOG ファイルへの書き込みが成功（新規追加、上流削除版の除去、または版順正規化。`changedFiles.length > 0`） |
...
| 4 | 検索時のハイライト（英語・日本語）は DOM テキストノードのみを `<mark class="highlight-token">` でラップ（属性値は変更しない） |
