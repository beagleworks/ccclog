...
#### 3.3.2 Release API からの取得
- リポジトリ: `anthropics/claude-code`
- 取得データ: リリース日（`published_at`）
- タグ形式: `v2.1.23` → バージョン `2.1.23`
- ページネーション: 100件/ページ、最大50ページ

#### 3.3.3 CHANGELOG.md コミット履歴からの取得
- `gh` CLI を使用して認証付きでアクセス
- CHANGELOG.md を変更したコミット履歴を取得
- 各コミットの差分から `+## X.Y.Z` パターンを検出
- コミット日時をリリース日として使用
- レート制限: 認証あり（gh CLI）で 5,000リクエスト/時間
...
#### 3.3.5 レート制限対応
- Release API 403エラー時: CHANGELOG.md コミット履歴にフォールバック
- コミット履歴も取得できない場合: 近隣バージョンから補間
...
| 日付 | バージョン | 変更内容 |
|-----|-----------|---------|
| 2026-01-29 | 1.2.0 | CHANGELOG.md コミット履歴からのリリース日取得を追加 |
