...
| 除外対象 | コードブロック、アセット列挙、空行のみのブロック、Changelog/PRs 行以降、未知セクション |
...
| Changelog/PRs 行以降 | `## Changelog`、`Changelog`、`Full Changelog:`、`PRs Merged` で始まる行で終了 |
| セクションヘッダー前の箇条書き | デフォルト `new-features`（実データでは発生しない） |

#### 2.6.4 見出しレベル
`##` および `###` に対応。`####` 以下はセクション切り替えとして認識しない。
